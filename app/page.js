    ...
    return `
      <div id="building-${index}" class="building-content" style="display: ${index === 0 ? 'block' : 'none'};">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px;">
          <div style="background: linear-gradient(135deg, #006a8e 0%, #008aae 100%); padding: 18px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0, 106, 142, 0.15);">
            <div style="font-size: 11px; color: rgba(255, 255, 255, 0.85); text-transform: uppercase; font-weight: 700; letter-spacing: 0.5px; margin-bottom: 8px;">Recommended Sum Insured</div>
            <div style="color: white; font-size: 22px; font-weight: 700; text-shadow: 0 1px 2px rgba(0,0,0,0.1);">${formatCurrency(item["Recommended Sum Insured ($)"])}</div>
          </div>

          <div style="background: white; padding: 18px; border-radius: 8px; border: 2px solid #006a8e; box-shadow: 0 2px 8px rgba(0, 106, 142, 0.1);">
            <div style="font-size: 11px; color: #006a8e; text-transform: uppercase; font-weight: 700; letter-spacing: 0.5px; margin-bottom: 8px;">Reinstatement Cost</div>
            <div style="color: #006a8e; font-size: 22px; font-weight: 700;">${formatCurrency(item["Reinstatement Cost\n($)"])}</div>
          </div>
        </div>

        <!-- Property Details -->
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 20px;">
          <div style="padding: 14px; background: #f0f9fc; border-radius: 8px; border-left: 4px solid #006a8e;">
            ...
            <div style="color: #0f172a; font-size: 14px; font-weight: 600;">${item["Entity"] || "N/A"}</div>
          </div>
          <div style="padding: 14px; background: #f0f9fc; border-radius: 8px; border-left: 4px solid #006a8e;">
            ...
            <div style="color: #0f172a; font-size: 14px; font-weight: 600;">${item["AVR ID"] || "N/A"}</div>
          </div>
        </div>

        <!-- Valuer Comments -->
        ${item["Valuer Comments"] && item["Valuer Comments"] !== 0 ? `
          <div style="margin-top: 18px; padding: 16px; background: #fffbeb; border-radius: 8px; border-left: 4px solid #f59e0b; box-shadow: 0 2px 6px rgba(245, 158, 11, 0.1);">
            <div style="font-size: 11px; color: #92400e; text-transform: uppercase; font-weight: 700; letter-spacing: 0.5px; margin-bottom: 8px; display: flex; align-items: center; gap: 6px;">
              <span style="font-size: 16px;">💬</span> Valuer Comments
            </div>
  ...
  function updateMarkers() {
    ...
    Object.entries(groupedBySite).forEach(([siteName, buildings]) => {
      ...
      if (buildings.length > 1) {
        tabsHTML = `
          <div style="display: flex; gap: 0; border-bottom: 2px solid #e2e8f0; background: #f0f9fc;">
            ${buildings.map((building, index) => `
              <button 
                ...
                style="
                  flex: 1;
                  padding: 14px 20px;
                  background: ${index === 0 ? 'white' : '#f0f9fc'};
                ...
                "
                onmouseover="if(document.getElementById('building-${index}').style.display === 'none') { this.style.background = '#e0f2f7'; this.style.color = '#006a8e'; }"
                onmouseout="if(document.getElementById('building-${index}').style.display === 'none') { this.style.background = '#f0f9fc'; this.style.color = '#64748b'; } else { this.style.background = 'white'; this.style.color = '#006a8e'; }"
              >
      ...
      const fullHTML = `
        <div style="padding: 18px 24px; background: linear-gradient(135deg, #006a8e 0%, #008aae 100%); border-bottom: 1px solid rgba(255,255,255,0.2);">
          <h3 style="margin: 0; font-size: 18px; font-weight: 700; color: white; letter-spacing: -0.2px; text-shadow: 0 1px 2px rgba(0,0,0,0.1);">
            ${siteName}
          </h3>
