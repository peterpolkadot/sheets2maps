...
async function getExcelRows() {
  ...
  // Try to get file info first to verify access
  const fileInfoUrl = "https://graph.microsoft.com/v1.0/drives/" + driveId + "/items/" + fileId;
  ...
  if (fileJson.error) {
    throw new Error("Cannot access file: " + JSON.stringify(fileJson.error) + " - Check EXCEL_DRIVE_ID and EXCEL_FILE_ID");
  }

  // Try to list worksheets
  const sheetsUrl = "https://graph.microsoft.com/v1.0/drives/" + driveId + "/items/" + fileId + "/workbook/worksheets";
  ...
  if (sheetsJson.error) {
    throw new Error("Cannot access worksheets: " + JSON.stringify(sheetsJson.error));
  }

  const sheetNames = (sheetsJson.value || []).map(s => s.name);
  
  if (!sheetNames.includes(sheet)) {
    throw new Error("Sheet '" + sheet + "' not found. Available sheets: " + sheetNames.join(", "));
  }
